...
class MovieRepository @Inject constructor(
    private val moviesApi: MovieApi, private val bookmarkDatabase: BookmarkDatabase
) {
    fun getMovies(): Flow<PagingData<com.example.movieapp.data.models.popular.Result>> {
        Log.d("TAG", "getMovies: ")
...
/*
    fun getNowPlaying(): Flow<PagingData<NowPlayingResult>> {
        return Pager(config = PagingConfig(
            pageSize = 2, enablePlaceholders = false, prefetchDistance = 20
        ), pagingSourceFactory = {
...
/*    fun getTopRatedPlaying(): Flow<PagingData<com.example.movieapp.data.models.toprated.Result>> {
        return Pager(config = PagingConfig(
            pageSize = 2, enablePlaceholders = false, prefetchDistance = 20
        ), pagingSourceFactory = {
    ...
    suspend fun insertSavedMovie(movie: com.example.movieapp.data.models.popular.Result) =
        bookmarkDatabase.bookMarkedMovieDao().insertItem(movie)

    suspend fun deleteSavedQuote(movieText: String) =
        bookmarkDatabase.bookMarkedMovieDao().deleteItem(movieText)
